...
const qdheader={
 ...
 if (!qdbody) {
    ...
    Object.keys(qdbodys).forEach((item) => {
        if (qdbodys[item]) {
            qdbodyArr.push(qdbodys[item])
        }
    })
...
!(async () => {
        ...
        console.log(`共${qdbodyArr.length}个账号`)
...
function getqdbody() {
    if ($request.url.match(/\/ant.xunsl.com\/v5\/CommonReward\/toGetReward/)) {
        ...
        if (qdbody) {
            if (qdbody.indexOf(bodyVal) > -1) {
                $.log("此签到请求已存在，本次跳过")
            } else if (qdbody.indexOf(bodyVal) == -1) {
                qdbodys = qdbody + "&" + bodyVal;
                $.setdata(qdbodys, 'qdbody');
                $.log(`${$.name}获取签到: 成功, qdbodys: ${bodyVal}`);
                bodys = qdbodys.split("&")
                $.msg($.name, "获取第" + bodys.length + "个签到请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'qdbody');
            $.log(`${$.name}获取签到: 成功, qdbodys: ${bodyVal}`);
